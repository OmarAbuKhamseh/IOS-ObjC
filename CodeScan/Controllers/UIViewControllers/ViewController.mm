    ...
    if(status == AVAuthorizationStatusAuthorized) {
        ...
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
            if(granted){
                ...
                UITapGestureRecognizer *shortTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapToFocus:)];    //tap gesturerecognizer
                shortTap.numberOfTapsRequired=1;
                shortTap.numberOfTouchesRequired=1;
                [self.view addGestureRecognizer:shortTap];
...
- (void)handleTapToFocus:(UITapGestureRecognizer *)tapGesture
{
    ...
    if (tapGesture.state == UIGestureRecognizerStateEnded)
    {
        ...
        if ([acd isFocusModeSupported:AVCaptureFocusModeAutoFocus] && [acd isFocusPointOfInterestSupported])
        {
            ...
            {
                [acd setFocusMode:AVCaptureFocusModeAutoFocus];   // focusmode
                [acd setFocusPointOfInterest:CGPointMake(focus_x, focus_y)];  // focus pointofinterest
...
- (void) Recog_Successed
{
    ...
    _matOrg.release();
    AudioServicesPlaySystemSound (1315);
    [self performSegueWithIdentifier:@"segue_ShowResult1" sender:self];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    //segue method
    if ([[segue identifier] isEqualToString:@"segue_ShowResult1"])
    {
